...
class Encoder(nn.Module):
    def __init__(self, num_classes):
        ...
        for x in range(0, 5):    #5 times
           self.layers.append(non_bottleneck_1d(64, 0.00, 1))#0.03   

        self.layers.append(DownsamplerBlock(64,128))

        for x in range(0, 2):    #2 times
            self.layers.append(non_bottleneck_1d(128, 0.0, 2))#0.3
            self.layers.append(non_bottleneck_1d(128, 0.0, 4))#0.3
            self.layers.append(non_bottleneck_1d(128, 0.0, 8))#0.3
            self.layers.append(non_bottleneck_1d(128, 0.0, 16))#0.3
...
class Decoder2 (nn.Module):
    ...
    def forward(self, x):
        return F.upsample(x,size=(512,1024), mode='bilinear') #train resolution: 512x512
...
class Net(nn.Module):
	...
	def forward(self, x, enc=True, finetune=False):
		...
		for seg_layer , up_layer , key in zip(self.seg , self.up , self.datasets):
			...
			y = up_layer(y)
			y = F.upsample(y,size=(512,1024), mode='bilinear')
			output_dict[key] = y
